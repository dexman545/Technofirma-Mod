...
public class ItemTechnofirmaIngot extends ItemTerra implements ISmeltable
{
	...
	private boolean CreatePile(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side, int l)
	{
		...
		if (world.getTileEntity(x, y, z) instanceof TETFIngotPile && world.getBlock(x,y,z) == TechnofirmaCore.TFIngotPile)
		{
		...
		if(fullStack && isPlaceable(itemstack))
		{
			...
			{
				world.setBlock( x, y-1, z, TechnofirmaCore.TFIngotPile, l, 0x2);
				if(world.isRemote) {
			...
			{
				world.setBlock( x, y, z-1, TechnofirmaCore.TFIngotPile, l, 0x2);
				if(world.isRemote) {
			...
			{
				world.setBlock( x-1, y, z, TechnofirmaCore.TFIngotPile, l, 0x2);
				if(world.isRemote) {
	...
	public boolean onItemUse(ItemStack itemstack, EntityPlayer entityplayer, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		if(entityplayer.isSneaking() && stackTagCompound == null && (itemstack.getItem().getUnlocalizedName().indexOf("Double")==-1) && this.isPlaceable(itemstack))
		{
			...
			if(!world.isRemote && entityplayer.isSneaking() && (world.getBlock(x, y, z) != TechnofirmaCore.TFIngotPile || (side != 1 && side != 0)))
			{
				...
				if(CreatePile(itemstack, entityplayer, world, x, y, z, side, dir))
				{
					...
					world.addBlockEvent(x,y,z,TechnofirmaCore.TFIngotPile,0,0);
					return true;

				}
			}
			else if(world.getBlock(x, y, z) == TechnofirmaCore.TFIngotPile)
			{
			...
			else
			{
				...
				{
					if (m>=16){
						world.setBlock(x, y+1, z, TechnofirmaCore.TFIngotPile, m, 0x2);
						itemstack.stackSize = itemstack.stackSize-1;
					}
					else{
						world.setBlock(x, y+1, z, TechnofirmaCore.TFIngotPile, m, 0x2);
	...
	{
		if(world.isSideSolid(i, j-1, k, ForgeDirection.UP) || world.getBlock(i, j-1, k)==TechnofirmaCore.TFIngotPile)
		{
	...
	public void setSide(World world, ItemStack itemstack, int m, int dir, int x, int y, int z, int i, int j, int k)
	{
		...
		{
			if(dir == 0 || dir == 2) {
				world.setBlock(x+i, y+j, z+k, TechnofirmaCore.TFIngotPile, m, 0x2);
			} else {
				world.setBlock(x+i, y+j, z+k, TechnofirmaCore.TFIngotPile, m | 8, 0x2);
			}
			itemstack.stackSize = itemstack.stackSize-1;
		}
		else if(m >= 16)
		{
			if(dir == 0 || dir == 2) {
				...
				world.setBlock(x+i, y+j, z+k, TechnofirmaCore.TFIngotPile, m-8 | 8, 0x2);
			}
			itemstack.stackSize = itemstack.stackSize-1;
		}
